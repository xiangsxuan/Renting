...
public partial class Admin_ChangePassword : System.Web.UI.Page
{
    ...
    protected void ChangePassword_Click(object sender, EventArgs e)
    {
        ...
        if (IsValid)
        {
            dr = sqlData.GetDataReader("select * from  Admin where AdminId='" + Session["AdminId"].ToString() + "'and Password='" + CurrentPassword.Text + "' ");
            if (dr.HasRows)
            {
                try
                {
                    sqlData.RunSql("Update Admin Set Password = '" + NewPassword.Text + "' where AdminId='" + Session["AdminId"].ToString() + "' ");
