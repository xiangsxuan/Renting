...
public partial class Tenant_SearchResult_Describe : System.Web.UI.Page
{
    ...
    public string getSearchResult()
    {
        ...
        sqlDataReader = sqlData.GetDataReader(@"select * from  House where Describe LIKE N'%" + Request["QueryKey"] + "%' or HouseTitle like N'%" + Request["QueryKey"] + "%'");
        ...
        while (sqlDataReader.Read())
        {
            if (no % 2 != 1)
            {
                retHtml += FrontTemple.getOddBoxHtml(int.Parse(sqlDataReader["HouseId"].ToString()), sqlDataReader["HouseTitle"].ToString(), sqlDataReader["Describe"].ToString(), Request["QueryKey"].ToString());
            }
            else
            {
                retHtml += FrontTemple.getEvenBoxHtml(int.Parse(sqlDataReader["HouseId"].ToString()), sqlDataReader["HouseTitle"].ToString(), sqlDataReader["Describe"].ToString(), Request["QueryKey"].ToString());
            }
            no++;
